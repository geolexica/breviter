  ...
  .command(
    ...
    argv => {
      process.stdout.write(
        `[breviter] Computing DB concept dataset at path "${argv.dataset}" ...`
      );
      savePrecomputedDB(argv.dataset, argv.output, argv.model).then(d => {
        process.stdout.write(
          `[breviter] Written precomputed DB to path "${argv.output}".`
        );
        // process.exit(0);
      });
    }
